...
func TestResolverIntents(t *testing.T) {
	...
	cases := []struct {
		...
		{"отмени задачу купить фильтр", ai.IntentTaskCancel},
		{"перенеси задачу купить фильтр на завтра", ai.IntentTaskRescheduleOne},
		{"задачи с тегом шопинг", ai.IntentTaskListByTag},
